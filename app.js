const CONFIG = {
  ...
  storageKey: 'js_proficiency_passport_ai_v11_professional_registry',
  adminPin: 'JS2026',
...
function buildDiagnosticItems(){
  ...
  const readingStems = [
    ['Reading','In an academic paragraph, the topic sentence usually presents the ___.','central idea','random example','font size','page number','Diagnostic Reading'],
    ['Reading','An inference question requires readers to use ___.','clues and logic','only the title','random guessing','the test password','Diagnostic Reading'],
    ['Reading','A reference question asks what a pronoun or phrase ___.','refers to','sounds like','costs','translates automatically','Diagnostic Reading'],
    ['Reading','When data support a claim, the data function as ___.','evidence','decoration','a greeting','a password','Diagnostic Reading'],
    ['Reading','The author’s purpose is the reason the writer ___.','presents the information','chooses the font','opens the door','counts the pages','Diagnostic Reading'],
    ['Reading','A contrast signal helps readers recognize ___.','difference','identical meaning','spelling only','audio quality','Diagnostic Reading'],
    ['Reading','The main idea of a passage is best described as ___.','the overall message','one minor detail','an unrelated opinion','the author’s address','Diagnostic Reading'],
    ['Reading','Vocabulary in context means understanding a word from ___.','surrounding text','the keyboard','the paper size','a random list','Diagnostic Reading'],
    ['Reading','Skimming is useful for finding ___.','general meaning','every comma','speaker accent','certificate number','Diagnostic Reading'],
    ['Reading','Scanning is useful for locating ___.','specific information','the author’s childhood','all hidden meanings','the test timer only','Diagnostic Reading'],
    ['Reading','A well-supported paragraph normally includes a claim and ___.','supporting details','unrelated jokes','empty spaces','QR codes only','Diagnostic Reading'],
    ['Reading','A conclusion usually ___.','summarizes or closes the discussion','introduces a new unrelated topic','removes the main idea','hides all evidence','Diagnostic Reading']
  ];
  const vocab = [
...
function buildItemBank(){
  ...
  SKILLS.forEach((skill,si)=>{
    for(let i=0;i<205;i++){
      ...
      bank.push({
        id:`EPC-ITEM-${itemNo}`, skill, subskill:sub[skill][i%sub[skill].length], level,
...
const PRELOADED_CANDIDATES = [
  {name:'Ahmad Fauzi Rahman', studentId:'STU-2026-0001', email:'candidate001@proficiency.local', department:'Informatics', score:507, diagnostic:442, mock:482},
  {name:'Siti Nur Aisyah', studentId:'STU-2026-0002', email:'candidate002@proficiency.local', department:'Management', score:544, diagnostic:479, mock:519},
  {name:'Budi Santoso', studentId:'STU-2026-0003', email:'candidate003@proficiency.local', department:'Nursing', score:581, diagnostic:516, mock:556},
  {name:'Dewi Lestari', studentId:'STU-2026-0004', email:'candidate004@proficiency.local', department:'Primary Teacher Education', score:618, diagnostic:553, mock:593},
  {name:'Rizky Pratama', studentId:'STU-2026-0005', email:'candidate005@proficiency.local', department:'Communication Studies', score:655, diagnostic:590, mock:630},
  {name:'Nurul Hidayah', studentId:'STU-2026-0006', email:'candidate006@proficiency.local', department:'English Education', score:692, diagnostic:627, mock:667},
  {name:'Agus Setiawan', studentId:'STU-2026-0007', email:'candidate007@proficiency.local', department:'Informatics', score:729, diagnostic:664, mock:704},
  {name:'Maya Anggraini', studentId:'STU-2026-0008', email:'candidate008@proficiency.local', department:'Management', score:766, diagnostic:701, mock:741},
  {name:'Rina Wulandari', studentId:'STU-2026-0009', email:'candidate009@proficiency.local', department:'Nursing', score:488, diagnostic:423, mock:463},
  {name:'Dimas Aditya', studentId:'STU-2026-0010', email:'candidate010@proficiency.local', department:'Primary Teacher Education', score:525, diagnostic:460, mock:500},
  {name:'Fitri Amalia', studentId:'STU-2026-0011', email:'candidate011@proficiency.local', department:'Communication Studies', score:562, diagnostic:497, mock:537},
  {name:'Arif Nugroho', studentId:'STU-2026-0012', email:'candidate012@proficiency.local', department:'English Education', score:599, diagnostic:534, mock:574},
  {name:'Laila Safitri', studentId:'STU-2026-0013', email:'candidate013@proficiency.local', department:'Informatics', score:636, diagnostic:571, mock:611},
  {name:'Hendra Wijaya', studentId:'STU-2026-0014', email:'candidate014@proficiency.local', department:'Management', score:673, diagnostic:608, mock:648},
  {name:'Putri Maharani', studentId:'STU-2026-0015', email:'candidate015@proficiency.local', department:'Nursing', score:710, diagnostic:645, mock:685},
  {name:'Fajar Kurniawan', studentId:'STU-2026-0016', email:'candidate016@proficiency.local', department:'Primary Teacher Education', score:747, diagnostic:682, mock:722},
  {name:'Nadia Permata', studentId:'STU-2026-0017', email:'candidate017@proficiency.local', department:'Communication Studies', score:784, diagnostic:719, mock:759},
  {name:'Yusuf Firmansyah', studentId:'STU-2026-0018', email:'candidate018@proficiency.local', department:'English Education', score:506, diagnostic:441, mock:481},
  {name:'Anisa Rahmawati', studentId:'STU-2026-0019', email:'candidate019@proficiency.local', department:'Informatics', score:543, diagnostic:478, mock:518},
  {name:'Bayu Saputra', studentId:'STU-2026-0020', email:'candidate020@proficiency.local', department:'Management', score:580, diagnostic:515, mock:555},
  {name:'Citra Ayu Ningrum', studentId:'STU-2026-0021', email:'candidate021@proficiency.local', department:'Nursing', score:617, diagnostic:552, mock:592},
  {name:'Eko Prasetyo', studentId:'STU-2026-0022', email:'candidate022@proficiency.local', department:'Primary Teacher Education', score:654, diagnostic:589, mock:629},
  {name:'Hanif Maulana', studentId:'STU-2026-0023', email:'candidate023@proficiency.local', department:'Communication Studies', score:691, diagnostic:626, mock:666},
  {name:'Intan Puspitasari', studentId:'STU-2026-0024', email:'candidate024@proficiency.local', department:'English Education', score:728, diagnostic:663, mock:703},
  {name:'Jihan Aulia', studentId:'STU-2026-0025', email:'candidate025@proficiency.local', department:'Informatics', score:765, diagnostic:700, mock:740},
  {name:'Kevin Arianto', studentId:'STU-2026-0026', email:'candidate026@proficiency.local', department:'Management', score:487, diagnostic:422, mock:462},
  {name:'Lukman Hakim', studentId:'STU-2026-0027', email:'candidate027@proficiency.local', department:'Nursing', score:524, diagnostic:459, mock:499},
  {name:'Miftahul Jannah', studentId:'STU-2026-0028', email:'candidate028@proficiency.local', department:'Primary Teacher Education', score:561, diagnostic:496, mock:536},
  {name:'Nabila Zahra', studentId:'STU-2026-0029', email:'candidate029@proficiency.local', department:'Communication Studies', score:598, diagnostic:533, mock:573},
  {name:'Oki Ramadhan', studentId:'STU-2026-0030', email:'candidate030@proficiency.local', department:'English Education', score:635, diagnostic:570, mock:610},
  {name:'Pandu Wicaksono', studentId:'STU-2026-0031', email:'candidate031@proficiency.local', department:'Informatics', score:672, diagnostic:607, mock:647},
  {name:'Qonita Salsabila', studentId:'STU-2026-0032', email:'candidate032@proficiency.local', department:'Management', score:709, diagnostic:644, mock:684},
  {name:'Rafi Alfarizi', studentId:'STU-2026-0033', email:'candidate033@proficiency.local', department:'Nursing', score:746, diagnostic:681, mock:721},
  {name:'Salsa Nurfadilah', studentId:'STU-2026-0034', email:'candidate034@proficiency.local', department:'Primary Teacher Education', score:783, diagnostic:718, mock:758},
  {name:'Taufik Hidayat', studentId:'STU-2026-0035', email:'candidate035@proficiency.local', department:'Communication Studies', score:505, diagnostic:440, mock:480},
  {name:'Umi Kalsum', studentId:'STU-2026-0036', email:'candidate036@proficiency.local', department:'English Education', score:542, diagnostic:477, mock:517},
  {name:'Vina Oktaviani', studentId:'STU-2026-0037', email:'candidate037@proficiency.local', department:'Informatics', score:579, diagnostic:514, mock:554},
  {name:'Wahyu Saputra', studentId:'STU-2026-0038', email:'candidate038@proficiency.local', department:'Management', score:616, diagnostic:551, mock:591},
  {name:'Yulia Kartika', studentId:'STU-2026-0039', email:'candidate039@proficiency.local', department:'Nursing', score:653, diagnostic:588, mock:628},
  {name:'Zainal Abidin', studentId:'STU-2026-0040', email:'candidate040@proficiency.local', department:'Primary Teacher Education', score:690, diagnostic:625, mock:665},
  {name:'Aulia Putra Ramadhan', studentId:'STU-2026-0041', email:'candidate041@proficiency.local', department:'Communication Studies', score:727, diagnostic:662, mock:702},
  {name:'Bella Kirana', studentId:'STU-2026-0042', email:'candidate042@proficiency.local', department:'English Education', score:764, diagnostic:699, mock:739},
  {name:'Cahyo Nugraha', studentId:'STU-2026-0043', email:'candidate043@proficiency.local', department:'Informatics', score:486, diagnostic:421, mock:461},
  {name:'Dian Pertiwi', studentId:'STU-2026-0044', email:'candidate044@proficiency.local', department:'Management', score:523, diagnostic:458, mock:498},
  {name:'Eka Aprilia', studentId:'STU-2026-0045', email:'candidate045@proficiency.local', department:'Nursing', score:560, diagnostic:495, mock:535},
  {name:'Farhan Akbar', studentId:'STU-2026-0046', email:'candidate046@proficiency.local', department:'Primary Teacher Education', score:597, diagnostic:532, mock:572},
  {name:'Gita Cahyani', studentId:'STU-2026-0047', email:'candidate047@proficiency.local', department:'Communication Studies', score:634, diagnostic:569, mock:609},
  {name:'Hafiz Irwansyah', studentId:'STU-2026-0048', email:'candidate048@proficiency.local', department:'English Education', score:671, diagnostic:606, mock:646},
  {name:'Indah Permatasari', studentId:'STU-2026-0049', email:'candidate049@proficiency.local', department:'Informatics', score:708, diagnostic:643, mock:683},
  {name:'Joko Prasetyo', studentId:'STU-2026-0050', email:'candidate050@proficiency.local', department:'Management', score:745, diagnostic:680, mock:720},
  {name:'Kartika Sari', studentId:'STU-2026-0051', email:'candidate051@proficiency.local', department:'Nursing', score:782, diagnostic:717, mock:757},
  {name:'Leo Mahendra', studentId:'STU-2026-0052', email:'candidate052@proficiency.local', department:'Primary Teacher Education', score:504, diagnostic:439, mock:479},
  {name:'Melati Kusuma', studentId:'STU-2026-0053', email:'candidate053@proficiency.local', department:'Communication Studies', score:541, diagnostic:476, mock:516},
  {name:'Nanda Febriansyah', studentId:'STU-2026-0054', email:'candidate054@proficiency.local', department:'English Education', score:578, diagnostic:513, mock:553},
  {name:'Olivia Putri', studentId:'STU-2026-0055', email:'candidate055@proficiency.local', department:'Informatics', score:615, diagnostic:550, mock:590},
  {name:'Putu Andika', studentId:'STU-2026-0056', email:'candidate056@proficiency.local', department:'Management', score:652, diagnostic:587, mock:627},
  {name:'Ratih Kusumaningrum', studentId:'STU-2026-0057', email:'candidate057@proficiency.local', department:'Nursing', score:689, diagnostic:624, mock:664},
  {name:'Satria Wijaya', studentId:'STU-2026-0058', email:'candidate058@proficiency.local', department:'Primary Teacher Education', score:726, diagnostic:661, mock:701},
  {name:'Tiara Paramitha', studentId:'STU-2026-0059', email:'candidate059@proficiency.local', department:'Communication Studies', score:763, diagnostic:698, mock:738},
  {name:'Usman Fadillah', studentId:'STU-2026-0060', email:'candidate060@proficiency.local', department:'English Education', score:485, diagnostic:420, mock:460},
  {name:'Vicky Hermawan', studentId:'STU-2026-0061', email:'candidate061@proficiency.local', department:'Informatics', score:522, diagnostic:457, mock:497},
  {name:'Winda Sari', studentId:'STU-2026-0062', email:'candidate062@proficiency.local', department:'Management', score:559, diagnostic:494, mock:534},
  {name:'Yogi Maulana', studentId:'STU-2026-0063', email:'candidate063@proficiency.local', department:'Nursing', score:596, diagnostic:531, mock:571},
  {name:'Zahra Nabila', studentId:'STU-2026-0064', email:'candidate064@proficiency.local', department:'Primary Teacher Education', score:633, diagnostic:568, mock:608},
  {name:'Aditya Firmansyah', studentId:'STU-2026-0065', email:'candidate065@proficiency.local', department:'Communication Studies', score:670, diagnostic:605, mock:645},
  {name:'Bunga Citra Lestari', studentId:'STU-2026-0066', email:'candidate066@proficiency.local', department:'English Education', score:707, diagnostic:642, mock:682},
  {name:'Chandra Saputra', studentId:'STU-2026-0067', email:'candidate067@proficiency.local', department:'Informatics', score:744, diagnostic:679, mock:719},
  {name:'Dwi Rahmawati', studentId:'STU-2026-0068', email:'candidate068@proficiency.local', department:'Management', score:781, diagnostic:716, mock:756},
  {name:'Erwin Maulana', studentId:'STU-2026-0069', email:'candidate069@proficiency.local', department:'Nursing', score:503, diagnostic:438, mock:478},
  {name:'Fitria Salsabila', studentId:'STU-2026-0070', email:'candidate070@proficiency.local', department:'Primary Teacher Education', score:540, diagnostic:475, mock:515},
  {name:'Galih Prakoso', studentId:'STU-2026-0071', email:'candidate071@proficiency.local', department:'Communication Studies', score:577, diagnostic:512, mock:552},
  {name:'Hana Maharani', studentId:'STU-2026-0072', email:'candidate072@proficiency.local', department:'English Education', score:614, diagnostic:549, mock:589},
  {name:'Iqbal Ramadhan', studentId:'STU-2026-0073', email:'candidate073@proficiency.local', department:'Informatics', score:651, diagnostic:586, mock:626},
  {name:'Jesica Marlina', studentId:'STU-2026-0074', email:'candidate074@proficiency.local', department:'Management', score:688, diagnostic:623, mock:663},
  {name:'Kurnia Dewi', studentId:'STU-2026-0075', email:'candidate075@proficiency.local', department:'Nursing', score:725, diagnostic:660, mock:700},
  {name:'Luthfi Hakim', studentId:'STU-2026-0076', email:'candidate076@proficiency.local', department:'Primary Teacher Education', score:762, diagnostic:697, mock:737},
  {name:'Mira Novitasari', studentId:'STU-2026-0077', email:'candidate077@proficiency.local', department:'Communication Studies', score:484, diagnostic:419, mock:459},
  {name:'Naufal Rizqullah', studentId:'STU-2026-0078', email:'candidate078@proficiency.local', department:'English Education', score:521, diagnostic:456, mock:496}
];


function candidateCodeFromIndex(i){return `EPC-CAN-${year()}-${String(1000+i).padStart(6,'0')}`}
function certCodeFromIndex(i){return `EPC-CERT-${year()}-${String(1000+i).padStart(6,'0')}`}
function buildPreloadedProfile(score){return Object.fromEntries(SKILLS.map((s,i)=>[s,Math.max(180,Math.min(860,score+(i-2)*17))]))}
function addPreloadedCandidates(base){
  PRELOADED_CANDIDATES.forEach((p,idx)=>{
    const n=idx+1;
    const reg=candidateCodeFromIndex(n);
    const certId=certCodeFromIndex(n);
    const band=cefrFromScore(p.score);
    const candidate={
      id:reg, registrationCode:reg, studentId:p.studentId, name:p.name, email:p.email,
      institution:'Cipta Wacana University', department:p.department, phone:'-', purpose:'Institutional English proficiency certification', role:'candidate', status:'active', createdAt:today(),
      diagnostic:{score:p.diagnostic, cefr:cefrFromScore(p.diagnostic).level, date:today(), profile:buildPreloadedProfile(p.diagnostic)},
      practice:Object.fromEntries(MODULES.map(m=>[m.id,true])),
      mock:{score:p.mock, date:today(), rawCorrect:Math.round(p.mock/900*50), total:50},
      realTest:{score:p.score, date:today(), integrity:'Verified', securityLogs:[], rawCorrect:Math.round(p.score/900*140), totalItems:140, sectionScores:{Listening:{correct:Math.round(p.score/900*50),total:50,minutes:35,scaled:p.score},Structure:{correct:Math.round(p.score/900*40),total:40,minutes:25,scaled:p.score},Reading:{correct:Math.round(p.score/900*50),total:50,minutes:55,scaled:p.score}}, format:'Institutional ITP-style: 50 Listening / 40 Structure / 50 Reading'},
      certificateId:certId
    };
    const cert={id:certId,candidateId:reg,registrationCode:reg,studentId:p.studentId,name:p.name,score:p.score,cefr:band.label,cefrLevel:band.level,conversion:etp(p.score),issued:today(),validUntil:String(Number(today().slice(0,4))+2)+today().slice(4),issuer:CONFIG.issuer,status:'Valid',integrity:'Verified',demo:false};
    base.accounts[p.email]={email:p.email,password:'proficiency123',role:'candidate',status:'approved',candidateId:reg,registrationCode:reg,studentId:p.studentId,name:p.name,institution:candidate.institution,department:p.department};
    base.candidates[reg]=candidate;
    base.certificates[certId]=cert;
  });
  base.lastSeq=Math.max(base.lastSeq||1, 1100);
  base.lastCertSeq=Math.max(base.lastCertSeq||1, 1100);
  return base;
}
function defaultState(){
  const demo = {
    id:'EPC-CAN-2026-000001', registrationCode:'EPC-CAN-2026-000001', studentId:'DEMO-ID-JS-0001', name:'Joko Slamet', email:'joko.demo@proficiency.local',
    ...
    practice:Object.fromEntries(MODULES.map(m=>[m.id,true])), mock:{score:742, date:today()}, realTest:{score:765, date:today(), integrity:'Verified', securityLogs:[]}, certificateId:'EPC-CERT-SAMPLE'
  };
  const cert = {id:'EPC-CERT-SAMPLE',candidateId:demo.id,registrationCode:demo.registrationCode,studentId:demo.studentId,name:demo.name,score:765,cefr:'C1 Advanced',cefrLevel:'C1',conversion:622,issued:today(),validUntil:String(Number(today().slice(0,4))+2)+today().slice(4),issuer:CONFIG.issuer,status:'Valid',integrity:'Verified',demo:true};
  const base = {
    ...
    accounts:{[demo.email]:{email:demo.email,password:'demo12345',role:'candidate',status:'approved',candidateId:demo.id,registrationCode:demo.registrationCode,studentId:demo.studentId,name:demo.name,institution:demo.institution,department:demo.department}},
    candidates:{[demo.id]:demo}, certificates:{[cert.id]:cert}, pending:[], logs:[], lastSeq:1, lastCertSeq:1,
    batches:[{id:'EPC-BATCH-2026-001',name:'Academic Proficiency Batch',date:today(),status:'Open'}]
  };
  return addPreloadedCandidates(base);
}
function migrateState(s){
  const fresh = defaultState();
  s.accounts = {...fresh.accounts, ...(s.accounts||{})};
  s.candidates = {...fresh.candidates, ...(s.candidates||{})};
  s.certificates = {...fresh.certificates, ...(s.certificates||{})};
  s.batches = s.batches && s.batches.length ? s.batches : fresh.batches;
  s.lastSeq = Math.max(s.lastSeq||1, fresh.lastSeq||1);
  s.lastCertSeq = Math.max(s.lastCertSeq||1, fresh.lastCertSeq||1);
  return s;
}
...
function regCode(role){return `EPC-${rolePrefix(role)}-${year()}-${seq()}`}
function certCode(){return `EPC-CERT-${year()}-${certSeq()}`}
...
function qrImageUrl(id){return `https://api.qrserver.com/v1/create-qr-code/?size=170x170&margin=10&data=${encodeURIComponent(verificationUrl(id))}`}
function createCandidateRecord(data, demo=false){
  const role=data.role||'candidate';
  const code=demo?'EPC-CAN-2026-000001':regCode(role);
  return {id:code, registrationCode:code, studentId:data.staffId||data.studentId||code, name:data.name||'Candidate', email:(data.email||'').toLowerCase(), institution:data.institution||'', department:data.department||'', phone:data.phone||'', purpose:data.purpose||'', role:'candidate', status:'active', createdAt:today(), diagnostic:null, practice:{}, mock:null, realTest:null, certificateId:null};
}
function makeCertificate(c, demo=false){
  const id = demo?'EPC-CERT-SAMPLE':certCode();
  const score = c.realTest?.score || c.mock?.score || c.diagnostic?.score || 0;
  const band = cefrFromScore(score);
  const cert={id,candidateId:c.id,registrationCode:c.registrationCode,studentId:c.studentId||c.staffId||c.registrationCode,name:c.name,score,cefr:band.label,cefrLevel:band.level,conversion:etp(score),issued:today(),validUntil:String(Number(today().slice(0,4))+2)+today().slice(4),issuer:CONFIG.issuer,status:'Valid',integrity:c.realTest?.integrity||'Verified',demo};
...
function quizItem(prefix,i,it){const isListening=it[0]==='Listening';const script=isListening?listeningScriptForItem(it,i):'';return `<div class="quiz-item ${isListening?'listening-item':''}"><div class="question-line"><b>${i+1}. [${it[0]}]</b>${it[6]?` <span class="pill blue">${esc(it[6])}</span>`:''}${isListening?` <span class="pill gold">${prefix==='r'?'One-time audio':'Audio controls available'}</span>`:''}</div>${audioButton(script,prefix,i,isListening)}<p>${esc(it[1])}</p><div class="options">${[2,3,4,5].map((idx,j)=>`<label class="option"><input type="radio" name="${prefix}${i}" value="${j===0?'correct':'wrong'}" required><span>${esc(it[idx])}</span></label>`).join('')}</div></div>`}
function renderITPItems(prefix, items){let last='';return items.map((it,i)=>{let header='';if(it[6]!==last){last=it[6];const audioText=prefix==='r'?'Official mode: Listening audio can be played one time only. Replay and stop controls are not available during the real test.':(it[0]==='Listening'?'Practice mode: use Play, Replay, and Stop controls as needed.':'Answer all questions in this section.');header=`<div class="section-break"><h3>${esc(it[6])}</h3><p>${audioText}</p></div>`}return header+quizItem(prefix,i,it)}).join('')}
...
function renderPractice(){
  ...
  return `<section class="section-title"><div><h2>AI Proficiency Preparation Studio</h2><p>Practice is personalized after diagnosis and may be completed before official testing. Listening practice includes Play, Replay, and Stop controls.</p></div><div class="actions"><button class="ghost" data-view="dashboard">Back to Dashboard</button><button class="primary" data-action="completeAllPractice">Complete Demo Practice</button></div></section>
...
function renderCertificate(cert, candidatePage=false){
  const verifyLink = verificationUrl(cert.id);
  return `<div class="certificate-wrap"><div class="certificate ${cert.demo?'':'official'}" id="certificateCard"><h2>CERTIFICATE OF ENGLISH<br>PROFICIENCY</h2><p class="center">This is to certify that</p><div class="cert-name">${esc(cert.name)}</div><div class="cert-sub">${cert.demo?'Demo / example certificate':'Official English Proficiency Credential'}</div><div class="cert-score"><span>${cert.score} / 900</span><span>${esc(cert.cefr)}</span></div><p class="center">Issued by<br><b>${esc(cert.issuer)}</b></p><div class="cert-meta"><div><span>Certificate ID</span><b>${esc(cert.id)}</b></div><div><span>Candidate ID</span><b>${esc(cert.studentId||cert.registrationCode)}</b></div><div><span>Registration Code</span><b>${esc(cert.registrationCode)}</b></div><div><span>Institutional Conversion</span><b>${cert.conversion}</b></div><div><span>Integrity Status</span><b>${esc(cert.integrity)}</b></div><div><span>Issued / Valid Until</span><b>${cert.issued} / ${cert.validUntil}</b></div></div><div class="cert-grid"><img class="qr-image" alt="Scannable certificate QR code" src="${qrImageUrl(cert.id)}"><div class="sign"><div class="scribble">Joko Slamet</div><small>Authorized Platform Director</small></div><div class="stamp">VALID<br>CREDENTIAL</div></div></div><aside class="cert-side"><div class="verify-panel"><h3>Certificate Verification</h3><p class="verify-status">${esc(cert.status)}</p><p><b>Certificate ID:</b><br>${esc(cert.id)}</p><p><b>Candidate ID:</b><br>${esc(cert.studentId||cert.registrationCode)}</p><p><b>Candidate:</b><br>${esc(cert.name)}</p><p><b>Score:</b><br>${cert.score}/900 • ${esc(cert.cefr)}</p><p><b>Verification Link:</b><br><a href="#verify/${encodeURIComponent(cert.id)}">Open public verification</a></p><p class="small">Scan the QR code to open the same verification record.</p><div class="actions"><button class="primary" data-action="downloadCert" data-cert="${esc(cert.id)}">Download Certificate</button><button class="ghost" data-action="printCert">Print / Save PDF</button>${candidatePage?`<button class="soft" data-view="report">Back to Report</button>`:''}</div></div></aside></div>`;
}
function renderCertificateRegistry(){
  ...
  return `<section class="section-title"><div><h2>Certificate Registry</h2><p>Official credential records with candidate ID, certificate code, score, integrity status, and QR verification access.</p></div><button class="ghost" data-view="dashboard">Back to Dashboard</button></section>
  <div class="card"><div class="table-wrap"><table><thead><tr><th>Certificate ID</th><th>Candidate</th><th>Candidate ID</th><th>Registration Code</th><th>Score</th><th>CEFR</th><th>Status</th><th>Verify</th></tr></thead><tbody>${certs.map(cert=>`<tr><td>${esc(cert.id)}</td><td>${esc(cert.name)}</td><td>${esc(cert.studentId||'—')}</td><td>${esc(cert.registrationCode)}</td><td>${cert.score}/900</td><td>${esc(cert.cefr)}</td><td><span class="pill ${cert.status==='Valid'?'green':'red'}">${esc(cert.status)}</span></td><td><a class="btn" href="#verify/${encodeURIComponent(cert.id)}">Open QR Record</a></td></tr>`).join('')}</tbody></table></div></div>`;
}
function renderAdmin(view){
  if(view==='approvals') return renderApprovals(); if(view==='users') return renderUsers(); if(view==='items') return renderItems(); if(view==='certificates') return renderCertificateRegistry(); if(view==='analytics') return renderAnalytics(); if(view==='proctoring') return renderProctoring(); return renderAdminDashboard();
}
function renderAdminDashboard(){
  ...
  <div class="grid grid-3">${[['Approvals','Approve lecturer and proctor registration requests.','approvals'],['Users & Candidates','Review registered users and integrated candidate codes.','users'],['Item Bank','Inspect 1000+ items across six English skills.','items'],['Certificate Registry','Open issued certificates and QR verification records.','certificates'],['Analytics','View CEFR distribution and institutional score readiness.','analytics'],['Proctoring','Review real-test security logs and integrity status.','proctoring'],['Certificate Demo','Open Joko Slamet certificate verification demo.','certificateDemo']].map(x=>`<div class="card module"><h3>${x[0]}</h3><p>${x[1]}</p><button class="primary" ${x[2]==='certificateDemo'?'data-action="verifySample"':`data-view="${x[2]}"`}>Open</button></div>`).join('')}</div>`;
}
...
function renderUsers(){
  ...
  <div class="card"><div class="table-wrap"><table><thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Status</th><th>Candidate/Staff ID</th><th>Registration Code</th><th>Institution</th></tr></thead><tbody>${accounts.map(a=>`<tr><td>${esc(a.name)}</td><td>${esc(a.email)}</td><td>${roleLabel(a.role)}</td><td><span class="pill ${a.status==='approved'?'green':a.status==='rejected'?'red':'gold'}">${esc(a.status)}</span></td><td>${esc(a.studentId||a.staffId||'—')}</td><td>${esc(a.registrationCode||'—')}</td><td>${esc(a.institution||'—')}</td></tr>`).join('')}</tbody></table></div></div>`;
}
...
function handleClick(e){
  ...
  if(action==='verifySample'){location.hash='#verify/EPC-CERT-SAMPLE';return}
...
function register(fd){
  ...
  if(role==='candidate'){
    const c=createCandidateRecord({name:fd.get('name'), email, role, staffId:fd.get('staffId'), institution:fd.get('institution'), department:fd.get('department'), phone:fd.get('phone'), purpose:fd.get('purpose')});
    c.registrationCode=code; c.id=code; state.candidates[c.id]=c; candidateId=c.id;
  }
  state.accounts[email]={email,password:String(fd.get('password')),role,status,name:String(fd.get('name')),staffId:String(fd.get('staffId')),studentId:String(fd.get('staffId')),phone:String(fd.get('phone')),institution:String(fd.get('institution')),department:String(fd.get('department')),purpose:String(fd.get('purpose')),registrationCode:code,candidateId,createdAt:today()};
  save();
...
function adminLogin(fd){if(String(fd.get('pin'))!==CONFIG.adminPin)return toast('Incorrect Admin PIN.');state.sessionUser={role:'admin',email:'admin@local',name:'Administrator',registrationCode:'EPC-ADM-'+year()+'-000001'};state.view='dashboard';save();render()}
