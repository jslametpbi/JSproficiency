const CONFIG = {
  ...
  subtitle: 'Field Testing Edition • 78 Participants',
  storageKey: 'js_proficiency_passport_ai_field_testing_78_v10',
  adminPin: 'JS2026',
...
function buildFieldParticipants(){
  const institutions=['Field Testing Cohort A','Field Testing Cohort B'];
  ...
  for(let i=1;i<=78;i++){
    ...
    const code=`EPP-FT-${year()}-${no}`;
    const certId=`EPP-CERT-FT-${year()}-${no}`;
    ...
    const logs=integrity==='Review Required'?[`Window focus warning during field session ${no}`]:[];
    const completedPractice=Object.fromEntries(MODULES.map((m,idx)=>[m.id, idx < (6 + (i%3))]));
    const c={
      id:code, registrationCode:code, name:`Field Participant ${no}`, email:`field.p${no}@proficiency.local`,
      ...
      purpose:'Field testing validation', role:'candidate', status:'active', createdAt:today(), fieldTest:true,
    ...
    fieldAccounts[c.email]={email:c.email,password:'field12345',role:'candidate',status:'approved',candidateId:code,registrationCode:code,name:c.name,institution:c.institution,department:c.department,fieldTest:true};
  }
...
function defaultState(){
  ...
  return {
    ...
    batches:[{id:'EPP-BATCH-FT-2026-001',name:'English Proficiency Field Testing Batch',date:today(),status:'Completed',participants:78,mode:'ITP-style secure field testing'}]
  };
}
let state = loadState();
let testTimer = null;
    ...
    <section class="landing">
      <div class="landing-left">
        <div>
          ...
          <p class="hero-desc">A complete AI-integrated field-testing platform cloned for 78 participants, integrating diagnostic profiling, practice preparation, ITP-style secure testing, score reporting, certificate verification, and institutional analytics.</p>
          <div class="cta-grid">
        ...
        <div class="metric-grid">
          <div class="metric"><span class="icon">◆</span><strong>78</strong><b>Field Participants</b><small>Preloaded cohort</small></div>
...
function renderSignInForm(){
  return `<div class="signin-split">
    <form id="loginForm" class="form form-2">
      ...
      <p class="small full">New user? <button class="link-button" type="button" data-action="authMode" data-mode="register">Create a detailed account</button>. Field-test participant demo: field.p001@proficiency.local / field12345.</p>
    </form>
...
function shellNav(){
  ...
  if(role==='candidate'){
    ...
    buttons.push(`<button class="soft" data-view="approvals">Approvals</button><button class="soft" data-view="users">Users</button><button class="soft" data-view="items">Item Bank</button><button class="soft" data-view="analytics">Analytics</button><button class="soft" data-view="fieldtest">Field Test</button><button class="soft" data-view="proctoring">Proctoring</button>`);
  } else if(role==='lecturer') {
    buttons.push(`<button class="soft" data-view="students">Students</button><button class="soft" data-view="items">Item Bank</button><button class="soft" data-view="analytics">Analytics</button><button class="soft" data-view="fieldtest">Field Test</button>`);
...
function renderAdmin(view){
  if(view==='approvals') return renderApprovals(); if(view==='users') return renderUsers(); if(view==='items') return renderItems(); if(view==='analytics') return renderAnalytics(); if(view==='fieldtest') return renderFieldTesting(); if(view==='proctoring') return renderProctoring(); return renderAdminDashboard();
}
...
function fieldParticipants(){return Object.values(state.candidates).filter(c=>c.fieldTest || String(c.registrationCode||'').startsWith('EPP-FT-'))}
...
function renderFieldTesting(){
  ...
  return `<section class="section-title"><div><h2>Field Testing Dashboard: 78 Participants</h2><p>Cloned platform edition with anonymized field-test participants, integrated registration codes, ITP-style scores, diagnostic profiles, integrity status, and QR certificate records.</p></div><div class="actions"><button class="ghost" data-view="dashboard">Back to Dashboard</button><button class="primary" data-action="exportFieldData">Export CSV</button></div></section>
  <div class="grid grid-4 mb"><div class="card stat"><span class="pill blue">Participants</span><strong>${data.length}</strong><span>Field-testing sample</span></div><div class="card stat"><span class="pill gold">Mean Score</span><strong>${mean}</strong><span>${cefrFromScore(mean).label}</span></div><div class="card stat"><span class="pill green">Verified</span><strong>${verified}</strong><span>Integrity approved</span></div><div class="card stat"><span class="pill red">Review</span><strong>${review}</strong><span>Sessions flagged</span></div></div>
...
function exportFieldData(){
  ...
  const blob=new Blob([csv],{type:'text/csv;charset=utf-8'}); const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='EPP_Field_Test_78_Participants.csv';a.click();URL.revokeObjectURL(a.href);toast('Field-test CSV exported.');
}
