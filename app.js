const CONFIG = {
  ...
  subtitle: 'Professional English Proficiency Platform',
  storageKey: 'js_proficiency_passport_ai_professional_cohort_v11',
  adminPin: 'JS2026',
...
const DIAGNOSTIC_BLUEPRINT = {
  totalItems:60,
  sections:[
    {skill:'Listening', items:12, purpose:'short conversations, academic talks, inference, details, and speaker purpose'},
    {skill:'Structure', items:12, purpose:'grammar control, clauses, agreement, and written expression'},
    {skill:'Reading', items:12, purpose:'main idea, detail, inference, reference, and vocabulary in context'},
    {skill:'Vocabulary', items:12, purpose:'academic words, collocation, synonyms, antonyms, and contextual meaning'},
    {skill:'Writing', items:6, purpose:'coherence, thesis control, grammar accuracy, and academic style'},
    {skill:'Speaking', items:6, purpose:'fluency, intelligibility, organization, pronunciation, and task response'}
  ]
};
function buildDiagnosticItems(){
  const listening = [
    ['Listening','What does the man imply about the assignment?','It must be submitted soon.','It has been cancelled.','It is optional.','It was already graded.','Diagnostic Listening: Short Conversations','Narrator: Listen to a short conversation. Woman: Have you finished the assignment for tomorrow? Man: I just checked the course page and realized the deadline is closer than I thought. Narrator: What does the man imply about the assignment?'],
    ['Listening','What is the woman probably going to do?','Register before the deadline.','Leave the university.','Cancel the workshop.','Ask for a new textbook.','Diagnostic Listening: Short Conversations','Narrator: Listen to a conversation. Man: The workshop registration closes at noon. Woman: Then I should complete the form before lunch. Narrator: What is the woman probably going to do?'],
    ['Listening','What problem does the student have?','The reading was difficult to finish.','The class was moved online.','The lecturer is absent.','The library is closed.','Diagnostic Listening: Short Conversations','Narrator: Listen to a conversation. Student: I barely finished the reading before class. Friend: It was much longer than everyone expected. Narrator: What problem does the student have?'],
    ['Listening','What does the speaker suggest?','Checking the notice board.','Ignoring the announcement.','Leaving campus immediately.','Submitting no form.','Diagnostic Listening: Short Conversations','Narrator: Listen to a campus conversation. Staff: The room number has changed. Please check the notice board before going upstairs. Narrator: What does the speaker suggest?'],
    ['Listening','What is the main purpose of the professor’s explanation?','To show how transitions organize a lecture.','To introduce a sports event.','To describe a payment system.','To criticize all students.','Diagnostic Listening: Academic Talks','Narrator: Listen to part of a lecture. Professor: Words such as however, therefore, and for example are important because they guide listeners through the organization of a talk. Narrator: What is the main purpose of the professor’s explanation?'],
    ['Listening','What does “on the other hand” signal?','Contrast.','Definition.','Repetition only.','Final score.','Diagnostic Listening: Academic Talks','Narrator: Listen to a lecture. Professor: Some researchers value speed in reading. On the other hand, others emphasize accuracy and deep understanding. Narrator: What does on the other hand signal?'],
    ['Listening','What should the student bring to the office?','Identification and payment receipt.','A dictionary only.','A laptop charger.','A printed novel.','Diagnostic Listening: Longer Conversations','Narrator: Listen to a conversation. Student: What should I bring after completing the online form? Staff: Bring your identification card and payment receipt to the office. Narrator: What should the student bring?'],
    ['Listening','What can be inferred about the laboratory?','It requires a reservation.','It is always closed.','It has no computers.','It replaces the final test.','Diagnostic Listening: Longer Conversations','Narrator: Listen to a campus conversation. Student: Can I use the language laboratory this afternoon? Technician: Yes, but you need to reserve a time slot first. Narrator: What can be inferred about the laboratory?'],
    ['Listening','What is the speaker mainly discussing?','The importance of validity evidence.','The cost of printing certificates.','A campus sports match.','The history of a library building.','Diagnostic Listening: Academic Talks','Narrator: Listen to part of a lecture. Professor: A test score is meaningful only when there is evidence that the test measures what it claims to measure. This is the basic concern of validity. Narrator: What is the speaker mainly discussing?'],
    ['Listening','Why does the woman say she will study in the library?','It is quieter than her dormitory.','It is closed today.','It has no internet.','It is too far from campus.','Diagnostic Listening: Short Conversations','Narrator: Listen to a conversation. Man: Are you studying in your room tonight? Woman: No, my roommates are too noisy, so I will go to the library. Narrator: Why does the woman say she will study in the library?'],
    ['Listening','What does the lecturer imply about the quiz?','Students should review the previous lesson.','The quiz will not be given.','Only absent students must take it.','It will be replaced by a party.','Diagnostic Listening: Academic Talks','Narrator: Listen to a lecturer. Professor: The short quiz will focus on what we covered last week, especially the examples at the end of the chapter. Narrator: What does the lecturer imply about the quiz?'],
    ['Listening','What does the conversation suggest about the student?','He needs technical help.','He has finished everything.','He is the course lecturer.','He wants to cancel the course.','Diagnostic Listening: Longer Conversations','Narrator: Listen to a conversation. Student: I cannot upload my file to the platform. Assistant: Please send me a screenshot, and I will check your account settings. Narrator: What does the conversation suggest about the student?']
  ];
  const structures = [
    ['Structure','The students ___ completed the diagnostic test.','have','has','having','to have','Diagnostic Structure'],
    ['Structure','The book ___ was recommended by the lecturer is useful.','that','what','whoever','where','Diagnostic Structure'],
    ['Structure','Neither the lecturer nor the students ___ late.','were','was','be','is','Diagnostic Structure'],
    ['Structure','The research findings ___ that active mediation improves learning.','suggest','suggests','suggesting','to suggest','Diagnostic Structure'],
    ['Structure','The candidate, along with two classmates, ___ ready for the test.','is','are','were','be','Diagnostic Structure'],
    ['Structure','The instrument ___ by three experts before pilot testing.','was validated','validated','was validating','has validate','Diagnostic Structure'],
    ['Structure','The more carefully students practise, ___ their performance becomes.','the stronger','stronger','strongest','the strongest','Diagnostic Structure'],
    ['Structure','A certificate should be issued only after the score ___ verified.','has been','have been','being','to be','Diagnostic Structure'],
    ['Structure','The new system is designed ___ cheating during online tests.','to reduce','reduce','reduced','reducing','Diagnostic Structure'],
    ['Structure','Had the student arrived earlier, she ___ the briefing.','would have attended','will attend','attends','attending','Diagnostic Structure'],
    ['Structure','Academic integrity requires that each candidate ___ independently.','work','works','working','to work','Diagnostic Structure'],
    ['Structure','The lecturer explained the rules so clearly ___ everyone understood them.','that','than','which','what','Diagnostic Structure']
  ];
  const diagnosticReadingPassages = [
    {
      title:'Diagnostic Reading Passage 1: Proficiency Score Interpretation',
      text:'A proficiency score is useful only when it is interpreted in relation to a clear purpose. A student who receives a score report should understand what the score means, which skills are strong, and which areas require further practice. For this reason, professional testing systems normally connect scores with descriptors, skill profiles, and learning recommendations.',
      items:[
        ['Reading','According to the passage, a proficiency score should be interpreted in relation to ___.','a clear purpose','a random certificate color','the student’s seating position','the number of browser tabs'],
        ['Reading','The passage suggests that a good score report should show ___.','strengths and areas for practice','only the candidate’s password','the price of the computer','unrelated personal information'],
        ['Reading','The word “descriptors” in the passage is closest in meaning to ___.','level explanations','decorative images','hidden files','audio buttons'],
        ['Reading','The author’s main purpose is to explain why score interpretation must be ___.','meaningful and connected to evidence','fast but unclear','private and impossible to verify','based only on guessing']
      ]
    },
    {
      title:'Diagnostic Reading Passage 2: Secure Online Testing',
      text:'A secure online test requires more than a timer and a set of questions. It should protect candidate identity, control item exposure, monitor suspicious activity, and keep a record of important events during the test session. These measures do not make cheating impossible, but they increase the trustworthiness of the final score.',
      items:[
        ['Reading','What is the main idea of the passage?','Secure online testing needs several protection layers.','Online testing only needs a countdown timer.','All online tests are impossible to administer.','Candidates should always use external websites.'],
        ['Reading','According to the passage, one purpose of security records is to ___.','keep evidence of important session events','replace all test questions','increase the size of the certificate','remove candidate identity'],
        ['Reading','What can be inferred from the passage?','Security systems reduce risk but cannot guarantee perfect prevention.','Timers alone can stop all forms of cheating.','Monitoring is unnecessary in online tests.','Item exposure has no effect on score trust.'],
        ['Reading','The phrase “item exposure” refers to ___.','how often or widely test questions are seen','the brightness of the screen','the candidate’s profile picture','the number of certificates printed']
      ]
    },
    {
      title:'Diagnostic Reading Passage 3: Academic Reading Strategies',
      text:'Effective readers do not read every text in exactly the same way. When they need the general meaning, they may skim the passage. When they need a date, name, number, or specific phrase, they may scan the text. For deeper comprehension, they connect details, transitions, and examples to the author’s central message.',
      items:[
        ['Reading','Skimming is mainly used to find ___.','general meaning','every punctuation mark','the test password','the certificate border'],
        ['Reading','Scanning is useful when readers need ___.','specific information','a new audio file','the author’s handwriting','all hidden meanings at once'],
        ['Reading','For deeper comprehension, readers should connect details to ___.','the author’s central message','the color of the page','the login button','a random answer choice'],
        ['Reading','The passage is mainly about ___.','different reading strategies for different purposes','why reading should be avoided','how to design a logo','how to print certificates']
      ]
    }
  ];
  const readingStems = diagnosticReadingPassages.flatMap(p => p.items.map(item => ['Reading', item[1], item[2], item[3], item[4], item[5], p.title, p.text]));
  const vocab = [
    ['Vocabulary','A synonym of significant is ___.','important','ordinary','unclear','late','Diagnostic Vocabulary'],
    ['Vocabulary','Valid in assessment means ___.','appropriate and supported','beautiful','fast','expensive','Diagnostic Vocabulary'],
    ['Vocabulary','Reliable means ___.','consistent','decorative','native-like','silent','Diagnostic Vocabulary'],
    ['Vocabulary','A synonym of demonstrate is ___.','show','hide','damage','forget','Diagnostic Vocabulary'],
    ['Vocabulary','The opposite of increase is ___.','decrease','improve','expand','continue','Diagnostic Vocabulary'],
    ['Vocabulary','The phrase score interpretation refers to ___.','the meaning assigned to test results','the color of the certificate','the room arrangement','the test password','Diagnostic Vocabulary'],
    ['Vocabulary','A collocation for research is ___.','conduct research','eat research','sleep research','paint research','Diagnostic Vocabulary'],
    ['Vocabulary','Academic means related to ___.','education and scholarship','shopping only','weather only','sports only','Diagnostic Vocabulary'],
    ['Vocabulary','Evidence is information used to ___.','support a claim','decorate a page','hide a result','replace a student','Diagnostic Vocabulary'],
    ['Vocabulary','A synonym of evaluate is ___.','assess','ignore','erase','whisper','Diagnostic Vocabulary'],
    ['Vocabulary','A word family of analyze includes ___.','analysis','animal','arrival','apology','Diagnostic Vocabulary'],
    ['Vocabulary','The phrase institutional use means use by ___.','an organization or institution','a random stranger','a private diary only','a music player','Diagnostic Vocabulary']
  ];
  const writing = [
    ['Writing','A strong academic paragraph should have ___.','a clear topic sentence and support','only one disconnected word','no organization','only emojis','Diagnostic Writing'],
    ['Writing','A thesis statement usually presents ___.','the main argument','the page number','a random greeting','the password','Diagnostic Writing'],
    ['Writing','Coherence in writing means that ideas are ___.','logically connected','hidden from readers','translated randomly','unrelated','Diagnostic Writing'],
    ['Writing','Cohesive devices include words such as ___.','however and therefore','table and chair','red and blue','one and two only','Diagnostic Writing'],
    ['Writing','Academic style normally avoids ___.','very informal slang','clear evidence','logical organization','precise vocabulary','Diagnostic Writing'],
    ['Writing','Revision is useful because it helps writers ___.','improve clarity and accuracy','delete all ideas','avoid feedback forever','remove the topic','Diagnostic Writing']
  ];
  const speaking = [
    ['Speaking','Intelligibility in speaking means listeners can ___.','understand the message','see the certificate','count the letters','change the password','Diagnostic Speaking'],
    ['Speaking','Fluency is most closely related to ___.','smoothness of speech','font size','page border','QR shape','Diagnostic Speaking'],
    ['Speaking','A good spoken response should be ___.','organized and relevant','completely silent','unrelated to the question','only one sound','Diagnostic Speaking'],
    ['Speaking','Pronunciation control supports communication by improving ___.','clarity','printing','login speed','certificate color','Diagnostic Speaking'],
    ['Speaking','When answering an opinion question, a speaker should give ___.','a reason and example','only a password','no explanation','an unrelated topic','Diagnostic Speaking'],
    ['Speaking','In ELF-oriented speaking assessment, the main priority is ___.','intelligibility','native-like accent only','silence','handwriting','Diagnostic Speaking']
  ];
  return [...listening, ...structures, ...readingStems, ...vocab, ...writing, ...speaking];
}
const DIAG_ITEMS = buildDiagnosticItems();
const TEST_ITEMS = [
...
function makeRealTestItems(){
  ...
  const passages=[
    {
      title:'Reading Passage 1: Score Interpretation and Test Use',
      text:'A proficiency test becomes valuable when its results are interpreted for a specific purpose. Institutions may use scores for placement, graduation screening, scholarship selection, or curriculum evaluation. However, a score alone is not enough. A responsible report should include skill profiles, performance descriptors, and clear cautions about the limits of the score. When test users understand these elements, they can make fairer academic decisions.',
      qs:[
        ['What is the main idea of the passage?','Proficiency scores must be interpreted responsibly for specific purposes.','All tests should be used for only one purpose.','Scores are more important than skill profiles.','Institutions should avoid using test reports.'],
        ['According to the passage, institutions may use proficiency scores for ___.','placement and graduation screening','choosing classroom furniture','removing all language courses','changing students’ birthdays'],
        ['What should a responsible score report include?','skill profiles and performance descriptors','only a large number','a hidden answer key','unrelated personal opinions'],
        ['The word “cautions” is closest in meaning to ___.','warnings or careful notes','decorations','final grades only','speaking scripts'],
        ['Why does the author mention placement and scholarship selection?','to give examples of possible test uses','to show that testing is never useful','to criticize all institutions','to explain how to print certificates'],
        ['What can be inferred from the passage?','A score can be misused if its limits are ignored.','A single number explains everything about proficiency.','Skill profiles are unnecessary in reports.','Curriculum evaluation cannot use test data.'],
        ['The phrase “these elements” refers to ___.','skill profiles, descriptors, and score cautions','passwords and browser settings','audio recordings and microphones only','student photos and logos'],
        ['The author’s attitude toward score use is best described as ___.','careful and responsible','careless and humorous','angry and unrelated','uncertain about all assessment'],
        ['Which statement is supported by the passage?','Test users need to understand score information.','Scores should never be explained to institutions.','Reports should hide skill weaknesses.','Scholarship selection must ignore English proficiency.'],
        ['The purpose of the passage is to ___.','explain responsible interpretation of proficiency scores','describe a listening test only','teach how to open a browser','compare different campus buildings']
      ]
    },
    {
      title:'Reading Passage 2: Security in Online Proficiency Testing',
      text:'Online testing offers flexibility, but it also creates new risks. Candidates may try to leave the test window, use another device, copy questions, or receive help from another person. For this reason, secure test platforms combine several protections: identity checking, restricted navigation, randomized items, camera monitoring, microphone monitoring, and post-test review. No single tool can prevent every form of cheating, so a layered security model is essential.',
      qs:[
        ['What is the passage mainly about?','the need for layered security in online testing','the history of printed dictionaries','the advantage of open browsing during tests','the design of classroom chairs'],
        ['Which risk is mentioned in the passage?','using another device during the test','arriving early for class','reading instructions carefully','answering independently'],
        ['Why are randomized items useful?','They reduce item exposure and predictable answer order.','They let candidates choose all answers freely.','They remove the need for identity checks.','They replace all proctoring decisions.'],
        ['The word “restricted” is closest in meaning to ___.','limited or controlled','expanded without limit','decorated','translated'],
        ['What can be inferred about test security?','It works best when several tools are combined.','It depends only on a timer.','It is unnecessary for online testing.','It should allow unlimited browser tabs.'],
        ['The phrase “post-test review” refers to ___.','checking records after the test session','studying before the test','printing the certificate first','deleting all logs immediately'],
        ['Which protection is NOT mentioned in the passage?','food delivery tracking','camera monitoring','identity checking','restricted navigation'],
        ['The author uses examples of cheating risks to ___.','justify the need for security measures','show that tests should be cancelled','argue for easier questions only','explain CEFR levels'],
        ['Which statement is true according to the passage?','No single tool can prevent every form of cheating.','Microphones alone solve all test security problems.','Online tests never need review.','Candidates should be allowed to copy questions.'],
        ['The author’s purpose is to ___.','explain why online tests require multiple protections','describe how to write an essay','promote open-book testing','criticize all digital platforms']
      ]
    },
    {
      title:'Reading Passage 3: Academic Reading Comprehension',
      text:'Academic reading requires more than recognizing familiar words. Readers must identify the main idea, separate major points from minor details, and notice how examples support a claim. They also need to understand references such as this, these, and they, because such words connect ideas across sentences. Strong readers use these connections to build a coherent understanding of the whole passage.',
      qs:[
        ['What is the main idea of the passage?','Academic reading involves connecting ideas and understanding organization.','Academic reading only requires memorizing words.','Examples are never important in academic texts.','Reference words should always be ignored.'],
        ['According to the passage, examples usually ___.','support a claim','replace the title','hide the main idea','remove coherence'],
        ['The word “coherent” is closest in meaning to ___.','logically connected','extremely short','unrelated','invisible'],
        ['Why are reference words important?','They connect ideas across sentences.','They show the page number.','They always introduce a new topic.','They identify the test password.'],
        ['What can be inferred about weak readers?','They may miss relationships among ideas.','They always understand references easily.','They never need to identify details.','They should ignore examples.'],
        ['The phrase “such words” refers to ___.','reference words like this, these, and they','all verbs in the passage','titles and subtitles only','technical passwords'],
        ['Which skill is mentioned in the passage?','separating major points from minor details','recording speaking responses','checking camera position','printing score reports'],
        ['The author’s purpose is to ___.','describe important academic reading skills','explain audio playback','teach certificate verification','argue against reading tests'],
        ['Which statement is supported by the passage?','Main ideas and details must be distinguished.','All details have equal importance.','Academic reading has no organization.','Familiar words are the only requirement for comprehension.'],
        ['The passage suggests that strong readers ___.','build understanding from connections among ideas','read only the first sentence','avoid all examples','translate every word separately']
      ]
    },
    {
      title:'Reading Passage 4: AI Support and Ethical Preparation',
      text:'Artificial intelligence can support language learning when it is used before the official test. During preparation, AI may explain grammar, suggest vocabulary, provide writing feedback, or generate practice questions. In the real test, however, AI assistance must be blocked because the score should represent the candidate’s own ability. The distinction between preparation support and test assistance is central to academic integrity.',
      qs:[
        ['What is the main idea of the passage?','AI can support preparation but must not assist candidates during the real test.','AI should answer all real test questions.','Preparation and testing have the same rules.','Grammar feedback is useless before a test.'],
        ['During preparation, AI may be used to ___.','explain grammar and suggest vocabulary','replace the official score','open external websites during the test','hide candidate identity'],
        ['Why must AI assistance be blocked in the real test?','The score should represent the candidate’s own ability.','AI is never useful for language learning.','The test should have no security rules.','Candidates should not practise before testing.'],
        ['The word “distinction” is closest in meaning to ___.','difference','certificate','microphone','password'],
        ['What can be inferred from the passage?','Ethical platforms separate learning support from assessment conditions.','AI feedback is acceptable during official test answering.','Academic integrity only concerns certificates.','Practice courses should be removed.'],
        ['Which AI use is acceptable according to the passage?','giving writing feedback before the real test','choosing answers during the real test','searching questions online during the test','receiving live answers from a chatbot'],
        ['The phrase “test assistance” refers to ___.','help used while taking the official test','a practice explanation before the test','general study advice','a diagnostic report'],
        ['The author’s purpose is to ___.','clarify ethical AI use in preparation and testing','advertise a grammar book','explain how to install software','reject all practice activities'],
        ['Which statement is true according to the passage?','Preparation support and real-test help are not the same.','AI should always be available during the real test.','Scores should represent group ability.','Vocabulary suggestions are forbidden before a test.'],
        ['Academic integrity is related to ___.','ensuring that official scores reflect independent performance','making certificates more colorful','allowing unlimited help during testing','removing all practice materials']
      ]
    },
    {
      title:'Reading Passage 5: Verifiable Certificates',
      text:'A language certificate is most trustworthy when institutions can verify it quickly. A secure certificate should include a unique code, candidate identity, test date, score, CEFR level, issuing authority, and integrity status. QR verification helps users confirm whether the certificate is valid, expired, revoked, or still under review. These features protect both the candidate and the institution from document misuse.',
      qs:[
        ['What is the passage mainly about?','features that make language certificates verifiable and trustworthy','ways to remove test dates from reports','why certificates should not include scores','how to avoid institutional verification'],
        ['Which certificate feature is mentioned in the passage?','a unique code','a hidden answer key','a random audio script','a browser history list'],
        ['QR verification helps users confirm ___.','certificate status','the candidate’s favorite subject','the number of practice modules','the price of a textbook'],
        ['The word “revoked” is closest in meaning to ___.','cancelled or withdrawn','decorated','translated','downloaded'],
        ['What can be inferred from the passage?','Verification reduces the risk of certificate misuse.','Certificates should be impossible to check.','Integrity status is unrelated to certification.','A test date should never be recorded.'],
        ['The phrase “these features” refers to ___.','certificate data and QR verification elements','listening questions only','grammar explanations','practice course modules'],
        ['Who benefits from verifiable certificates?','both candidates and institutions','only website designers','only microphone users','no one'],
        ['Why does the passage mention valid, expired, revoked, and under review?','to show possible certificate statuses','to explain reading question types','to compare listening accents','to list CEFR skill names'],
        ['Which statement is supported by the passage?','Issuing authority and integrity status should appear on a secure certificate.','A certificate should hide the candidate identity.','QR verification prevents all language errors.','Scores should be replaced by passwords.'],
        ['The author’s purpose is to ___.','explain how verification strengthens certificate trust','describe classroom seating','teach pronunciation practice','argue against official certificates']
      ]
    }
  ];
  passages.forEach((passage,p)=>{
    passage.qs.forEach((q,qIdx)=>{
      items.push(['Reading',`Question ${qIdx+1}: ${q[0]}`,q[1],q[2],q[3],q[4],passage.title,passage.text]);
    });
  });
...
function buildFieldParticipants(){
  const institutions=['Institutional Cohort A','Institutional Cohort B'];
  const departments=['English Education','Informatics','Management','Nursing','Primary Education','Communication'];
  const fieldAccounts={};
  const fieldCandidates={};
  const fieldCertificates={};
  const skillNames=['Listening','Structure','Reading','Vocabulary','Writing','Speaking'];
  for(let i=1;i<=78;i++){
    const no=String(i).padStart(3,'0');
    const code=`EPP-CAN-${year()}-001${no}`;
    const certId=`EPP-CERT-${year()}-001${no}`;
    const base=Math.min(790, Math.max(365, 405 + ((i*23)%305) + Math.floor(i/14)*18 - (i%5===0?34:0)));
    const score=base;
    const profile={};
    skillNames.forEach((sk,idx)=>{profile[sk]=Math.min(900,Math.max(220,score + ((i*(idx+5))%86)-43))});
    const integrity=i%19===0?'Review Required':'Verified';
    const logs=integrity==='Review Required'?[`Window focus warning during secure session ${no}`]:[];
    const completedPractice=Object.fromEntries(MODULES.map((m,idx)=>[m.id, idx < (6 + (i%3))]));
    const c={
      id:code, registrationCode:code, name:`Participant ${no}`, email:`participant.p${no}@proficiency.local`,
      institution:institutions[(i-1)%institutions.length], department:departments[(i-1)%departments.length], phone:`08${String(1000000000+i)}`,
      purpose:'Institutional proficiency validation', role:'candidate', status:'active', createdAt:today(), fieldTest:true,
      diagnostic:{score:Math.max(250,score-35), cefr:cefrFromScore(Math.max(250,score-35)).level, date:today(), profile},
      practice:completedPractice,
      mock:{score:Math.max(250,score-18), date:today()},
      realTest:{score, date:today(), integrity, securityLogs:logs,
        listeningCorrect:Math.round(score/900*50), structureCorrect:Math.round(score/900*40), readingCorrect:Math.round(score/900*50), rawCorrect:Math.round(score/900*140)},
      certificateId:certId
    };
    const band=cefrFromScore(score);
    const cert={id:certId,candidateId:code,registrationCode:code,name:c.name,score,cefr:band.label,cefrLevel:band.level,conversion:etp(score),issued:today(),validUntil:String(Number(today().slice(0,4))+2)+today().slice(4),issuer:CONFIG.issuer,status:integrity==='Verified'?'Valid':'Under Review',integrity,demo:false,fieldTest:true};
    fieldCandidates[code]=c;
    fieldCertificates[certId]=cert;
    fieldAccounts[c.email]={email:c.email,password:'participant12345',role:'candidate',status:'approved',candidateId:code,registrationCode:code,name:c.name,institution:c.institution,department:c.department,fieldTest:true};
  }
  return {fieldAccounts, fieldCandidates, fieldCertificates};
}
...
function defaultState(){
  const demo = {
    ...
    practice:Object.fromEntries(MODULES.map(m=>[m.id,true])), mock:{score:742, date:today()}, realTest:{score:765, date:today(), integrity:'Verified', securityLogs:[], listeningCorrect:43, structureCorrect:35, readingCorrect:44, rawCorrect:122}, certificateId:'EPP-CERT-SAMPLE'
  };
  ...
  const seed=buildFieldParticipants();
  return {
    ...
    accounts:{[demo.email]:{email:demo.email,password:'demo12345',role:'candidate',status:'approved',candidateId:demo.id,registrationCode:demo.registrationCode,name:demo.name,institution:demo.institution,department:demo.department},...seed.fieldAccounts},
    candidates:{[demo.id]:demo,...seed.fieldCandidates}, certificates:{[cert.id]:cert,...seed.fieldCertificates}, pending:[], logs:[], lastSeq:80, lastCertSeq:80,
    batches:[{id:'EPP-BATCH-2026-000001',name:'English Proficiency Institutional Cohort',date:today(),status:'Completed',participants:78,mode:'ITP-style secure cohort assessment'}]
  };
}
let state = loadState();
let testTimer = null;
    ...
    <section class="landing">
      <div class="landing-left">
        <div>
          ...
          <p class="hero-desc">A complete AI-integrated professional English proficiency platform, integrating diagnostic profiling, practice preparation, ITP-style secure testing, score reporting, certificate verification, and institutional analytics.</p>
          <div class="cta-grid">
        ...
        <div class="metric-grid">
          <div class="metric"><span class="icon">◆</span><strong>6</strong><b>Core Engines</b><small>Complete assessment suite</small></div>
...
function renderSignInForm(){
  return `<div class="signin-split">
    <form id="loginForm" class="form form-2">
      ...
      <p class="small full">New user? <button class="link-button" type="button" data-action="authMode" data-mode="register">Create a detailed account</button>. Demo candidate: joko.demo@proficiency.local / demo12345.</p>
    </form>
...
function shellNav(){
  ...
  if(role==='candidate'){
    ...
    buttons.push(`<button class="soft" data-view="approvals">Approvals</button><button class="soft" data-view="users">Users</button><button class="soft" data-view="items">Item Bank</button><button class="soft" data-view="analytics">Analytics</button><button class="soft" data-view="certificates">Certificates</button><button class="soft" data-view="fieldtest">Cohort Records</button><button class="soft" data-view="proctoring">Proctoring</button>`);
  } else if(role==='lecturer') {
    buttons.push(`<button class="soft" data-view="students">Students</button><button class="soft" data-view="items">Item Bank</button><button class="soft" data-view="analytics">Analytics</button><button class="soft" data-view="certificates">Certificates</button><button class="soft" data-view="fieldtest">Cohort Records</button>`);
...
function renderDiagnostic(){
  const c=currentCandidate();
  return `<section class="section-title"><div><h2>AI Diagnostic Level Test</h2><p>Standard 60-item diagnostic profile across Listening, Structure, Reading, Vocabulary, Writing, and Speaking. Listening items include Play, Replay, and Stop controls.</p></div><button class="ghost" data-view="dashboard">Back to Dashboard</button></section>
  <div class="grid grid-6 mb">${DIAGNOSTIC_BLUEPRINT.sections.map(sec=>`<div class="card stat"><span class="pill gold">${sec.skill}</span><strong>${sec.items}</strong><span>items</span></div>`).join('')}</div>
  <form id="diagnosticForm" class="card">
    ${renderITPItems('d',DIAG_ITEMS)}
...
function listeningScriptForItem(it,i){
  if(it[7]) return it[7];
  const stem=(it[1]||'').replace(/[_]+/g,'blank');
  const fallbacks=[
    'Narrator: Listen to a short conversation. Woman: The assignment must be submitted before class tomorrow. Man: I did not realize the deadline was so close. Narrator: Answer the question based on what the speakers imply.',
    'Narrator: Listen to part of a lecture. Professor: Transition words such as however, therefore, and for example help listeners follow the organization of an academic talk. Narrator: Answer the question based on the lecture.',
    'Narrator: Listen to a campus conversation. Student: I need to finish the online form first, then bring my identification card to the office. Staff: That is correct. Narrator: Answer the question based on the conversation.',
    'Narrator: Listen to an academic talk. Professor: The main point is usually supported by details, examples, and explanations. Good listeners separate the central idea from supporting information. Narrator: Answer the question based on the talk.'
  ];
  return `${fallbacks[i%fallbacks.length]} Question: ${stem}`;
}
function audioButton(script, prefix, i, isListening){
  if(!isListening) return '';
  const safeScript=script||'Narrator: Listen carefully and answer the question.';
  const encoded=encodeURIComponent(safeScript);
  const key=`${prefix}${i}`;
  const isReal=prefix==='r';
  if(isReal){
    const c=currentCandidate();
    const played=!!(c?.realAudioPlayed?.[key]);
    return `<div class="audio-panel audio-panel-real no-print" data-audio-key="${key}">
      <div class="audio-label"><span class="speaker-mark">🔊</span><div><b>Official Listening Audio</b><small>One-time play only. Replay and stop are disabled in the real test.</small></div></div>
      <button class="audio-btn real-once" type="button" data-action="playOnceAudio" data-audio="${encoded}" data-key="${key}" ${played?'disabled':''} aria-label="Play listening item ${i+1} one time only">${played?'🔒 Audio Used / Locked':'▶ Play Audio Once'}</button>
      <span class="audio-status ${played?'used':'ready'}">${played?'Played once and locked':'Ready — listen carefully before answering'}</span>
    </div>`;
  }
  return `<div class="audio-panel audio-panel-practice no-print" data-audio-key="${key}">
    <div class="audio-label"><span class="speaker-mark">🔊</span><div><b>Listening Audio Controls</b><small>Practice/diagnostic audio may be played, replayed, and stopped.</small></div></div>
    <button class="audio-btn" type="button" data-action="playAudio" data-audio="${encoded}" data-key="${key}" aria-label="Play listening item ${i+1}">▶ Play Audio</button>
    <button class="audio-btn replay" type="button" data-action="replayAudio" data-audio="${encoded}" data-key="${key}" aria-label="Replay listening item ${i+1}">↻ Replay Audio</button>
    <button class="audio-btn stop" type="button" data-action="stopAudio" aria-label="Stop listening audio">■ Stop Audio</button>
    <span class="audio-status ready">Ready for repeated practice</span>
  </div>`;
}
function readingPassageHTML(it, passageNo){
  const title=it[6]||`Reading Passage ${passageNo}`;
  const text=it[7]||'Read the passage carefully before answering the questions below.';
  return `<article class="reading-passage" id="reading-passage-${passageNo}">
    <div class="reading-passage-head"><span class="pill gold">Reading Text</span><h3>${esc(title)}</h3></div>
    <p>${esc(text)}</p>
    <div class="reading-note">Answer all questions based only on this passage. The answer buttons below are fully clickable.</div>
  </article>`;
}
function quizItem(prefix,i,it){
  const isListening=it[0]==='Listening';
  const isReading=it[0]==='Reading';
  const script=isListening?listeningScriptForItem(it,i):'';
  const itemClass=`quiz-item ${isListening?'listening-item':''} ${isReading?'reading-item':''}`;
  const instruction=isReading?'<div class="reading-question-tag">Question based on the reading text above</div>':'';
  return `<div class="${itemClass}"><div class="question-line"><b>${i+1}. [${it[0]}]</b>${it[6]?` <span class="pill blue">${esc(it[6])}</span>`:''}${isListening?` <span class="pill gold">${prefix==='r'?'One-time audio':'Audio controls available'}</span>`:''}${isReading?` <span class="pill green">Passage-based item</span>`:''}</div>${audioButton(script,prefix,i,isListening)}${instruction}<p class="question-stem">${esc(it[1])}</p><div class="options clickable-options">${[2,3,4,5].map((idx,j)=>`<label class="option choice-btn" tabindex="0"><input type="radio" name="${prefix}${i}" value="${j===0?'correct':'wrong'}" required><span class="choice-letter">${String.fromCharCode(65+j)}</span><span class="choice-text">${esc(it[idx])}</span></label>`).join('')}</div></div>`
}
function renderITPItems(prefix, items){
  let lastSection='';
  let lastPassage='';
  let passageNo=0;
  return items.map((it,i)=>{
    ...
    if(it[6]!==lastSection){
      lastSection=it[6];
      const audioText=prefix==='r'?'Official mode: Listening audio can be played one time only. Replay and stop controls are not available during the real test.':(it[0]==='Listening'?'Practice mode: use Play, Replay, and Stop controls as needed.':(it[0]==='Reading'?'Read each passage first, then answer the passage-based questions with the clickable answer buttons.':'Answer all questions in this section.'));
      html+=`<div class="section-break"><h3>${esc(it[6]||it[0])}</h3><p>${audioText}</p></div>`;
    }
    if(it[0]==='Reading' && it[7] && it[7]!==lastPassage){
      lastPassage=it[7];
      passageNo++;
      html+=readingPassageHTML(it,passageNo);
    }
    html+=quizItem(prefix,i,it);
    return html;
  }).join('')
}
let currentUtterance = null;
function speakText(encoded, mode='practice'){
  const text=decodeURIComponent(encoded||'');
  if(!text)return toast('No audio script available.');
  if(!('speechSynthesis' in window))return toast('This browser does not support in-browser audio playback. Please use Safari, Chrome, or Edge with sound enabled.');
  window.speechSynthesis.cancel();
  const u=new SpeechSynthesisUtterance(text);
  currentUtterance=u; u.lang='en-US';u.rate=.86;u.pitch=1;u.volume=1;
  u.onstart=()=>toast(mode==='real'?'Official audio is playing once. Listen carefully.':mode==='replay'?'Replaying listening audio...':'Playing listening audio...');
  u.onend=()=>{currentUtterance=null};
  u.onerror=()=>toast('Audio playback was blocked. Click the audio button again or check browser sound permission.');
  window.speechSynthesis.speak(u);
}
function playAudio(encoded){speakText(encoded,'practice')}
function replayAudio(encoded){speakText(encoded,'replay')}
function stopAudio(){if('speechSynthesis' in window){window.speechSynthesis.cancel();currentUtterance=null;toast('Audio stopped.')}}
function playOnceAudio(encoded,key,btn){
  const c=currentCandidate(); if(!c)return;
  c.realAudioPlayed=c.realAudioPlayed||{};
  if(c.realAudioPlayed[key]){toast('This real-test audio has already been played once and is locked.');return}
  c.realAudioPlayed[key]=true;
  logSecurity(`One-time listening audio played: ${key}`);
  save();
  if(btn){btn.disabled=true;btn.innerHTML='🔒 Audio Used / Locked';const panel=btn.closest('.audio-panel');const status=panel?.querySelector('.audio-status');if(status){status.textContent='Played once and locked';status.classList.remove('ready');status.classList.add('used')}}
  speakText(encoded,'real');
}
function renderPractice(){
  const c=currentCandidate(); const p=completion(c);
  const practiceListen=makeRealTestItems().filter(x=>x[0]==='Listening').slice(0,8);
  const practiceRead=makeRealTestItems().filter(x=>x[0]==='Reading').slice(0,10);
  return `<section class="section-title"><div><h2>AI Proficiency Preparation Studio</h2><p>Practice is personalized after diagnosis and may be completed before official testing. Listening practice includes Play, Replay, and Stop controls; reading practice includes full passages and clickable choices.</p></div><div class="actions"><button class="ghost" data-view="dashboard">Back to Dashboard</button><button class="primary" data-action="completeAllPractice">Complete Demo Practice</button></div></section>
  <div class="card mb"><div class="progress"><span style="width:${p}%"></span></div><p class="small">${p}% completed</p></div>
  <div class="card mb practice-audio-lab"><h3>Listening Practice Audio Lab</h3><p>Use these controls to practise before the real test. In practice, every listening item has <b>Play Audio</b>, <b>Replay Audio</b>, and <b>Stop Audio</b>. In the real test, each audio can be played only once.</p>${renderITPItems('p',practiceListen)}</div>
  <div class="card mb reading-practice-lab"><h3>Reading Practice Lab</h3><p>Read the full passage and answer the clickable answer choices. This prepares candidates for the 50-item real Reading Comprehension section.</p>${renderITPItems('pr',practiceRead)}</div>
...
function renderMock(){
  ...
  <div class="card mb"><h3>Mock Test Blueprint</h3><p>15 original ITP-style demo items: Listening 5, Structure 5, Reading 5. Mock Listening provides Play, Replay, and Stop controls for preparation.</p></div>
...
function renderRealTest(){
  ...
  <div class="card mb"><h3>Official Institutional ITP-Style Test Directions</h3><p>This demo uses original institutional items and does not reproduce official TOEFL/ETS content. The structure follows the common ITP-style proportion: Listening Comprehension, Structure and Written Expression, and Reading Comprehension. Listening audio is generated inside the browser and is restricted to one play per item in the real test. Replay and Stop are available only in Diagnosis, Practice, and Mock modes.</p></div>
...
function renderAdmin(view){
  if(view==='approvals') return renderApprovals(); if(view==='users') return renderUsers(); if(view==='items') return renderItems(); if(view==='analytics') return renderAnalytics(); if(view==='certificates') return renderCertificateRegistry(); if(view==='fieldtest') return renderFieldTesting(); if(view==='proctoring') return renderProctoring(); return renderAdminDashboard();
}
...
function fieldParticipants(){return Object.values(state.candidates).filter(c=>c.fieldTest)}
function avg(nums){return Math.round(nums.reduce((a,b)=>a+Number(b||0),0)/Math.max(1,nums.length))}
function renderFieldTesting(){
  const data=fieldParticipants();
  const scores=data.map(c=>c.realTest?.score||0).filter(Boolean);
  const mean=avg(scores); const min=Math.min(...scores); const max=Math.max(...scores);
  const verified=data.filter(c=>c.realTest?.integrity==='Verified').length;
  const review=data.length-verified;
  const byLevel={}; data.forEach(c=>{const label=cefrFromScore(c.realTest?.score||0).level;byLevel[label]=(byLevel[label]||0)+1});
  const skillAvg={Listening:avg(data.map(c=>c.diagnostic?.profile?.Listening||0)),Structure:avg(data.map(c=>c.diagnostic?.profile?.Structure||0)),Reading:avg(data.map(c=>c.diagnostic?.profile?.Reading||0)),Vocabulary:avg(data.map(c=>c.diagnostic?.profile?.Vocabulary||0)),Writing:avg(data.map(c=>c.diagnostic?.profile?.Writing||0)),Speaking:avg(data.map(c=>c.diagnostic?.profile?.Speaking||0))};
  return `<section class="section-title"><div><h2>Institutional Cohort Dashboard</h2><p>Professional cohort workspace with anonymized participant records, integrated registration codes, ITP-style scores, diagnostic profiles, integrity status, and QR-verifiable certificates.</p></div><div class="actions"><button class="ghost" data-view="dashboard">Back to Dashboard</button><button class="primary" data-action="exportFieldData">Export CSV</button></div></section>
  <div class="grid grid-4 mb"><div class="card stat"><span class="pill blue">Participants</span><strong>${data.length}</strong><span>Cohort sample</span></div><div class="card stat"><span class="pill gold">Mean Score</span><strong>${mean}</strong><span>${cefrFromScore(mean).label}</span></div><div class="card stat"><span class="pill green">Verified</span><strong>${verified}</strong><span>Integrity approved</span></div><div class="card stat"><span class="pill red">Review</span><strong>${review}</strong><span>Sessions flagged</span></div></div>
  <div class="grid grid-3 mb"><div class="card"><h3>Score Range</h3><p class="bigline">${min}–${max}</p><p class="small">Overall EPP score scale: 0–900 with institutional conversion.</p></div><div class="card"><h3>CEFR Distribution</h3><div class="tag-cloud">${Object.entries(byLevel).map(([k,v])=>`<span class="pill gold">${k}: ${v}</span>`).join('')}</div></div><div class="card"><h3>Skill Mean Profile</h3><div class="mini-bars">${Object.entries(skillAvg).map(([k,v])=>`<div><span>${k}</span><b>${v}</b><i style="width:${Math.min(100,Math.round(v/9))}%"></i></div>`).join('')}</div></div></div>
  <div class="card mb"><h3>Cohort Assessment Protocol</h3><p>The platform is prepared for a 78-participant institutional cohort using the complete pathway: AI Diagnostic Level Test, Practice Course, Mock Test, Secure Real Test, Score Report, Certificate, Proctoring Review, and Analytics. Participant names are anonymized for ethical reporting.</p></div>
  <div class="card"><h3>Participant Records</h3><div class="table-wrap"><table><thead><tr><th>No.</th><th>Participant</th><th>Registration Code</th><th>Institution/Cohort</th><th>Department</th><th>Score</th><th>CEFR</th><th>Conversion</th><th>Integrity</th><th>Certificate</th></tr></thead><tbody>${data.map((c,i)=>{const cert=state.certificates[c.certificateId];const score=c.realTest?.score||0;return `<tr><td>${i+1}</td><td>${esc(c.name)}</td><td>${esc(c.registrationCode)}</td><td>${esc(c.institution)}</td><td>${esc(c.department)}</td><td>${score}</td><td>${cefrFromScore(score).label}</td><td>${etp(score)}</td><td><span class="pill ${c.realTest?.integrity==='Verified'?'green':'gold'}">${esc(c.realTest?.integrity||'Pending')}</span></td><td><a href="#verify/${encodeURIComponent(cert?.id||'')}">${esc(cert?.id||'—')}</a></td></tr>`}).join('')}</tbody></table></div><p class="small">Cohort demo login pattern: participant.p001@proficiency.local to participant.p078@proficiency.local; password: participant12345.</p></div>`;
}


function renderCertificateRegistry(){
  const certs=Object.values(state.certificates).sort((a,b)=>String(a.id).localeCompare(String(b.id)));
  return `<section class="section-title"><div><h2>Certificate Registry</h2><p>Official credential workspace for opening, verifying, printing, and downloading issued English proficiency certificates.</p></div><div class="actions"><button class="ghost" data-view="dashboard">Back to Dashboard</button><button class="primary" data-action="verifySample">Open Sample Verification</button></div></section>
  <div class="grid grid-4 mb"><div class="card stat"><span class="pill green">Issued</span><strong>${certs.length}</strong><span>Certificate records</span></div><div class="card stat"><span class="pill gold">Valid</span><strong>${certs.filter(c=>c.status==='Valid').length}</strong><span>Approved credentials</span></div><div class="card stat"><span class="pill red">Review</span><strong>${certs.filter(c=>c.status!=='Valid').length}</strong><span>Under review</span></div><div class="card stat"><span class="pill blue">Issuer</span><strong>CWCU</strong><span>Official testing center</span></div></div>
  <div class="card"><h3>Issued Certificates</h3><div class="table-wrap"><table><thead><tr><th>Certificate ID</th><th>Candidate</th><th>Registration Code</th><th>Score</th><th>CEFR</th><th>Integrity</th><th>Actions</th></tr></thead><tbody>${certs.map(cert=>`<tr><td>${esc(cert.id)}</td><td>${esc(cert.name)}</td><td>${esc(cert.registrationCode)}</td><td>${cert.score}</td><td>${esc(cert.cefr)}</td><td><span class="pill ${cert.integrity==='Verified'?'green':'gold'}">${esc(cert.integrity)}</span></td><td><a class="small-link" href="#verify/${encodeURIComponent(cert.id)}">Verify</a> <button class="mini" data-action="downloadCert" data-cert="${esc(cert.id)}">Download</button></td></tr>`).join('')}</tbody></table></div></div>`;
}
...
function renderLecturer(view){ if(view==='students')return renderUsers(); if(view==='items')return renderItems(); if(view==='analytics')return renderAnalytics(); if(view==='certificates')return renderCertificateRegistry(); if(view==='fieldtest')return renderFieldTesting(); return `<section class="section-title"><div><h2>Lecturer Dashboard</h2><p>Monitor candidate readiness, practice completion, item bank coverage, and score analytics.</p></div><span class="pill green">Approved Lecturer</span></section><div class="grid grid-3"><div class="card module"><h3>Students</h3><p>View registered candidates and readiness.</p><button class="primary" data-view="students">Open Students</button></div><div class="card module"><h3>Item Bank</h3><p>Review item bank coverage by skill.</p><button class="primary" data-view="items">Open Item Bank</button></div><div class="card module"><h3>Analytics</h3><p>Review CEFR distribution and score data.</p><button class="primary" data-view="analytics">Open Analytics</button></div><div class="card module"><h3>Certificates</h3><p>Open issued cohort certificates and verification records.</p><button class="primary" data-view="certificates">Open Certificates</button></div></div>`}
...
function exportFieldData(){
  const rows=[['No','Participant','Email','Registration Code','Institution','Department','Diagnostic Score','Mock Score','Real Test Score','CEFR','Institutional Conversion','Listening Correct','Structure Correct','Reading Correct','Raw Correct','Integrity','Certificate ID']];
  fieldParticipants().forEach((c,i)=>rows.push([i+1,c.name,c.email,c.registrationCode,c.institution,c.department,c.diagnostic?.score||'',c.mock?.score||'',c.realTest?.score||'',cefrFromScore(c.realTest?.score||0).label,etp(c.realTest?.score||0),c.realTest?.listeningCorrect||'',c.realTest?.structureCorrect||'',c.realTest?.readingCorrect||'',c.realTest?.rawCorrect||'',c.realTest?.integrity||'',c.certificateId||'']));
  const csv=rows.map(r=>r.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n');
  const blob=new Blob([csv],{type:'text/csv;charset=utf-8'}); const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='English_Proficiency_Cohort_78_Participants.csv';a.click();URL.revokeObjectURL(a.href);toast('Cohort CSV exported.');
}
...
function handleClick(e){
  ...
  if(action==='exportFieldData'){exportFieldData();return}
  if(action==='flagIntegrity'){setIntegrity(b.dataset.cid,'Review Required');return}
  if(action==='playOnceAudio'){playOnceAudio(b.dataset.audio,b.dataset.key,b);return}
  if(action==='playAudio'){playAudio(b.dataset.audio);return}
  if(action==='replayAudio'){replayAudio(b.dataset.audio);return}
...
function submitReal(fd){stopAudio();const items=makeRealTestItems();let correct=0;const sectionScores={Listening:{correct:0,total:0,minutes:35},Structure:{correct:0,total:0,minutes:25},Reading:{correct:0,total:0,minutes:55}};items.forEach((it,i)=>{sectionScores[it[0]].total++; if(fd.get('r'+i)==='correct'){correct++;sectionScores[it[0]].correct++}});Object.values(sectionScores).forEach(v=>v.scaled=sectionScore(v.correct,v.total));const score=Math.round(correct/items.length*900);const c=currentCandidate();const logs=c.realTest?.securityLogs||[];c.realTest={score,date:today(),integrity:logs.length?'Review Required':'Verified',securityLogs:logs,rawCorrect:correct,totalItems:items.length,sectionScores,format:'ITP-style: 50 Listening / 40 Structure / 50 Reading'};makeCertificate(c,false);save();toast(`Official ITP-style test submitted: ${correct}/${items.length} correct, ${score}/900. Certificate generated.`);detachSecurity();navTo('certificate')}
